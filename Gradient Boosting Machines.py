import pandas as pd
from sklearn.ensemble import GradientBoostingClassifier  # Import GBM Classifier
...
#author hemanth,dharshan,nandheeswaran
...
gbm_classifier = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=1, random_state=42)  # Use GBM Classifier here

# Train the model
gbm_classifier.fit(X_train_tfidf, y_train)

# Predictions on validation set
y_val_pred = gbm_classifier.predict(X_val_tfidf)
...
for max_depth in max_depth_range:
    gbm_classifier = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=max_depth, random_state=42)  # Use GBM Classifier here
    gbm_classifier.fit(X_train_tfidf, y_train)
    
    # Predictions on training set
    y_train_pred = gbm_classifier.predict(X_train_tfidf)
...
plt.ylabel('Accuracy')
plt.title('Train and Test Accuracy for GBM')
plt.legend()
plt.show()
